...
export const getTemplateHeaderNode = (cb) => {
  const node = document.createElement(`div`);
  node.innerHTML = getTemplateHeader();
...
const getTemplateField = (gameState) => {
  return `
...
export const getTemplateFieldNode = (gameState) => {
  const node = document.createElement(`div`);
  node.innerHTML = getTemplateField(gameState);
  return node;
};
